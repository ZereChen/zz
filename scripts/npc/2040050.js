...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        cm.sendSimple("   Hi~#b#h ##k 我是流浪炼金术士，我会制作一些稀有的道具，你有没有想要了解一下的呢？\r\n\r\n#b#L0#" + cm.显示物品(4006000) + "#l\r\n#b#L1#" + cm.显示物品(4006001) + "#l");
    } else if (status == 2) {
        set = selection;
        ...
        cm.sendSimple("    只有我能做#b#t" + makeitem + "##k这神秘的石头. 最近很多冒险者说需要这个. 做#t" + makeitem + "#的办法就是这些材料了，你要用什么材料制作呢?" + menu);
    } else if (status == 3) {
        if (selection < 0) {
            cm.dispose();
            return;
        ...
        cm.sendYesNo("    做#b5个#t" + makeitem + "##k需要如下的材料，大多是打猎怪物就会得到的。所以你努力下去，不那么难得到。怎么样? 你真想做吗?\r\n" + menu);
    } else if (status == 4) {
        for (i = 0; i < reqitem.length; i++) {
